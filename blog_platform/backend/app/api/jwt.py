...
def _token_exc() -> HTTPException:
    return HTTPException(status_code=401, detail="token无效或已过期")


def _forbbiden_exc() -> HTTPException:
    return HTTPException(status_code=403, detail="您没有权限执行此操作")
